...
function packageVariant(
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [entrypoint, "build", ...runnerArguments],
      {
        cwd: desktopRoot,
        env: {
          ...process.env,
          ...
          VITE_BUSINESS_APP_ORIGIN: "https://business.example.test",
          VITE_LIFE_DOCK_ENABLED: "true",
          VITE_LIFE_APP_ORIGIN: "https://life.example.test",
        },
        stdio: "inherit",
      },
    );
...
test("Pacioli packaging retains Dock CSP and isolated frontend artifacts", async () => {
  const result = path.join(
    tmpdir(),
    `pacioli-tauri-config-${process.pid}.json`,
  );
  await packageVariant(
    "0",
    result,
    ["--", "--locked"],
    path.join(desktopRoot, "scripts/tauri-business-dock.mjs"),
  );
  const invocation = JSON.parse(readFileSync(result, "utf8"));
  const configs = invocation.args.flatMap((arg, index) =>
    arg === "--config" ? [JSON.parse(invocation.args[index + 1])] : [],
  );
  const csp = configs.find((config) => config.app?.security?.csp)?.app.security
    .csp;
  assert.ok(
    csp.includes(
      "frame-src 'self' https://business.example.test https://life.example.test",
    ),
  );
  assert.equal(invocation.observed, "0");
  assert.ok(!path.isAbsolute(invocation.configured));
  assert.ok(
    invocation.args.lastIndexOf("--config") < invocation.args.indexOf("--"),
  );
});
