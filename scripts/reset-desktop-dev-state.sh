...
# Production state (`com.shiyueshizi.pacioli`, `~/.pacioli`, and `pacioli-desktop`) is
...
remove_bundle_state() {
  ...
  [[ -d "$base" ]] || return 0
  shopt -s nullglob
  for prefix in com.shiyueshizi.pacioli.dev; do
...
case "$(uname -s)" in
  Darwin)
    ...
    if command -v security >/dev/null 2>&1; then
      while security delete-generic-password -s pacioli-desktop-dev >/dev/null 2>&1; do :; done
    fi
    ;;
  Linux)
...
: > "$HOME/.pacioli-dev/.dev-nest-migrated"

log "Development desktop state removed; production Pacioli and Buzz state were not touched"
