...
BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"com.shiyueshizi.pacioli.dev\",\"productName\":\"Pacioli Dev\"}"
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    if [[ -n "$GIT_COMMON_DIR" && "$GIT_DIR" != "$GIT_COMMON_DIR" ]]; then
        ...
        if [[ "${BUZZ_SHARE_IDENTITY:-0}" == "1" ]]; then
            KEYRING_SERVICE="pacioli-desktop-dev"
            KEYRING_BLOB=""
            case "$(uname -s)" in
                Darwin)
            ...
            CANONICAL_KEY="$HOME/Library/Application Support/com.shiyueshizi.pacioli.dev/identity.key"
            ...
            if [[ -n "$SHARED_IDENTITY" ]]; then
                ...
                echo "⚠ BUZZ_SHARE_IDENTITY=1 but no Pacioli identity found in keyring service $KEYRING_SERVICE or at $CANONICAL_KEY — run Pacioli from repo root first" >&2
            fi
        fi
        ...
        if swift "$GENERATE_DEV_ICON" "$BASE_ICON" "$DEV_ICON" "$BUZZ_WORKTREE_LABEL"; then
            ...
            BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"com.shiyueshizi.pacioli.dev.${BUZZ_INSTANCE_SLUG}\",\"productName\":\"Pacioli Dev (${BUZZ_WORKTREE_LABEL})\",\"bundle\":{\"icon\":[\"$DEV_ICON\"]}}"
        fi
    fi
fi
