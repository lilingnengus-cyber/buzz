...
async fn cancelled_turn_with_usage_emits_notification_before_response() {
    ...
    // Send cancel, but keep round 2 blocked until the agent acknowledges it.
    // Flushing stdin alone does not prove the agent has processed cancellation.
    ...
    for _ in 0..40 {
        let v = h.recv().await;
        if v["id"] == json!(c_id) {
            assert!(v.get("error").is_none(), "cancel must be acknowledged: {v}");
            saw_cancel_ok = true;
