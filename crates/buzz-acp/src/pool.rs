...
impl<'a> TurnRuntimePolicy<'a> {
    fn resolve(context: &'a PromptContext, extension: Option<&TurnPolicy>) -> Self {
        ...
        Self {
            ...
            base_prompt: extension.base_prompt.or(context.base_prompt.as_deref()),
...
pub async fn run_prompt_task(
    ...
    let source_channel_id = batch.as_ref().map(|batch| batch.scope.channel_id());
    let conversation = match source_channel_id {
        Some(channel_id) => VerifiedConversation::Channel {
            channel_id,
            channel_type: resolved_channel_info
                .as_ref()
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    fn turn_runtime_policy_preserves_defaults_and_applies_extension_overrides() {
        ...
        context.base_prompt = Some("default prompt".into());
