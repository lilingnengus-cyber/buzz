...
pub(super) async fn check(
    ...
    for (family, name) in [
        ...
        ("operational_adjustment", "reversal"),
                ...
                let result = store
                    .verify_agent_delegation(
                        VerifyAgentDelegationRequest {
                            ...
                            trace_id: trace,
                            used_calls: 1,
                            required_scope: if mode == 6 {
                                "operating_report_snapshot_intent:approve".into()
                            } else {
                                scope.clone()
                            },
                            approval: if mode == 5 { None } else { Some(candidate) },
                        },
                        facts(trace),
                    )
                    .await;
