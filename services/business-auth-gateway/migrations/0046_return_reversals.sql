-- Original return lines and movements remain immutable evidence after correction.
ALTER TABLE sales_returns DROP CONSTRAINT sales_returns_status_check,
 ADD CONSTRAINT sales_returns_status_check CHECK(status IN ('draft','confirmed','cancelled','reversed'));
ALTER TABLE purchase_returns DROP CONSTRAINT purchase_returns_status_check,
 ADD CONSTRAINT purchase_returns_status_check CHECK(status IN ('draft','confirmed','cancelled','reversed'));
ALTER TABLE inventory_movements DROP CONSTRAINT inventory_movements_movement_type_check,
 ...
 'opening_balance','opening_balance_reversal','sales_shipment','sales_shipment_reversal',
 'purchase_receipt','purchase_receipt_reversal','sales_return','purchase_return',
 'sales_return_scrap','inventory_count_adjustment','sales_return_reversal',
 'sales_return_scrap_reversal','purchase_return_reversal'));

ALTER TABLE business_agent_return_disposition_intents DROP CONSTRAINT business_agent_return_disposition_intents_kind_check,
ADD CONSTRAINT business_agent_return_disposition_intents_kind_check CHECK(kind IN ('sales_return_inspection_intent','purchase_return_dispatch_intent','purchase_return_acknowledgment_intent','sales_return_cancellation_intent','purchase_return_cancellation_intent','sales_return_reversal_intent','purchase_return_reversal_intent'));
