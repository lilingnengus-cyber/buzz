CREATE TABLE business_agent_crm_intents (
 id uuid PRIMARY KEY,
 kind text NOT NULL CHECK(kind IN ('crm_creation_intent','crm_update_intent','crm_followup_intent')),
...
CREATE TRIGGER business_agent_crm_intents_immutable
 BEFORE UPDATE OR DELETE ON business_agent_crm_intents
...
ALTER TABLE business_document_approval_requests DROP CONSTRAINT business_document_approval_requests_document_type_check,
 ...
 ADD CONSTRAINT business_document_approval_requests_action_code_check CHECK(action_code IN ('sales_order:confirm','purchase_order:confirm','shipment:confirm','goods_receipt:confirm','inventory_opening:post','customer_receipt:confirm','supplier_payment:confirm','receivable_allocation:create','payable_allocation:create','customer_receipt:reverse','supplier_payment:reverse','receivable_allocation:reverse','payable_allocation:reverse','sales_order:cancel','purchase_order:cancel_remaining','shipment:reverse','goods_receipt:reverse','inventory_opening:reverse','inventory_opening:create','crm:manage'));
...
FROM (VALUES ('crm_creation_intent'),('crm_update_intent'),('crm_followup_intent')) AS kinds(kind)
