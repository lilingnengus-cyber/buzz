CREATE TABLE business_agent_inventory_count_creation_intents (
 id uuid PRIMARY KEY,
 kind text NOT NULL CHECK(kind='inventory_count_creation_intent'),
...
CREATE TRIGGER business_agent_inventory_count_creation_intents_immutable
 BEFORE UPDATE OR DELETE ON business_agent_inventory_count_creation_intents
...
SELECT gen_random_uuid(),'inventory_count_creation_intent:'||action,'inventory_count_creation_intent',action,
CASE WHEN action='approve' THEN '["fresh_signed_chat_command"]'::jsonb ELSE '[]'::jsonb END,'high'
FROM (VALUES ('create'),('approve')) AS capability(action)
