CREATE TABLE business_agent_adjustment_intents (
 id uuid PRIMARY KEY,
 kind text NOT NULL CHECK(kind IN ('operational_adjustment_post_intent')),
...
FROM (VALUES ('operational_adjustment_post_intent')) kinds(kind)
CROSS JOIN (VALUES ('create'),('approve')) actions(action)
ON CONFLICT(capability) DO NOTHING;
-- No automatic grants or approval policies.

ALTER TABLE business_document_approval_requests DROP CONSTRAINT business_document_approval_requests_action_code_check,
 ADD CONSTRAINT business_document_approval_requests_action_code_check CHECK(action_code IN ('sales_order:confirm','purchase_order:confirm','shipment:confirm','goods_receipt:confirm','inventory_opening:post','customer_receipt:confirm','supplier_payment:confirm','receivable_allocation:create','payable_allocation:create','customer_receipt:reverse','supplier_payment:reverse','receivable_allocation:reverse','payable_allocation:reverse','sales_order:cancel','purchase_order:cancel_remaining','shipment:reverse','goods_receipt:reverse','inventory_opening:reverse','inventory_opening:create','crm:manage','business_master_data:manage','business_product_master:manage','sales_order:place_hold','sales_order:release_hold','management_report:generate_snapshot','profit_adjustment:post'));
