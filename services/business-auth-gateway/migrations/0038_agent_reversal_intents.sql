CREATE TABLE business_agent_reversal_intents (
 id uuid PRIMARY KEY,
 kind text NOT NULL CHECK(kind IN ('customer_receipt_reversal_intent','supplier_payment_reversal_intent','receivable_allocation_reversal_intent','payable_allocation_reversal_intent')),
...
CREATE TRIGGER business_agent_reversal_intents_immutable BEFORE UPDATE OR DELETE ON business_agent_reversal_intents FOR EACH ROW EXECUTE FUNCTION deny_business_document_approval_vote_mutation();
...
SELECT gen_random_uuid(),resource||':'||action,resource,action,
 ...
 'high'
FROM (VALUES ('customer_receipt_reversal_intent','create'),('customer_receipt_reversal_intent','approve'),('supplier_payment_reversal_intent','create'),('supplier_payment_reversal_intent','approve'),('receivable_allocation_reversal_intent','create'),('receivable_allocation_reversal_intent','approve'),('payable_allocation_reversal_intent','create'),('payable_allocation_reversal_intent','approve')) AS capability(resource,action)
