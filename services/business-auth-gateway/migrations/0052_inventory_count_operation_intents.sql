CREATE TABLE business_agent_inventory_count_operation_intents (
 id uuid PRIMARY KEY,
 kind text NOT NULL CHECK(kind IN ('inventory_count_submission_intent','inventory_count_posting_intent','inventory_count_cancellation_intent')),
 inventory_count_id uuid NOT NULL REFERENCES inventory_count_tasks(id),
...
CREATE TRIGGER business_agent_inventory_count_operation_intents_immutable
 BEFORE UPDATE OR DELETE ON business_agent_inventory_count_operation_intents
...
FROM (VALUES ('inventory_count_submission_intent'),('inventory_count_posting_intent'),('inventory_count_cancellation_intent')) AS resource(resource)
CROSS JOIN (VALUES ('create'),('approve')) AS operation(action)
