...
pub(super) async fn search(
    ...
    let Ok(mut input) = serde_json::from_value::<SearchFinancialDocumentsInput>(input.clone())
    else {
    ...
    let Ok(mut url) = core
        .base_url
        .join(&format!("v1/agent-financial-documents/{kind}"))
    else {
    ...
    Json(BusinessToolResult {
        ...
        summary: BTreeMap::from([
            ("source".into(), json!("business-core-financial-documents")),
        ...
        resource_refs: items
            ...
            .filter_map(|item| {
                let (resource, id) = match kind {
                    "customer_receipt" => ("customer-receipt", item["id"].as_str()?),
                    "supplier_payment" => ("supplier-payment", item["id"].as_str()?),
...
mod tests {
    ...
    async fn lookup_forwards_identity_and_preserves_source_pagination() {
        ...
        let server = Router::new().route("/v1/agent-financial-documents/customer_receipt", get(move |headers: HeaderMap, Query(query): Query<HashMap<String,String>>| async move {
