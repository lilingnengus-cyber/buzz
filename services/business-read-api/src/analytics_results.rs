...
pub(super) async fn core_profit_risk_result(
    core: &CoreClient,
    tool: &str,
    input: &Value,
...
pub(super) async fn core_profit_change_result(
    core: &CoreClient,
    input: &Value,
    scope: &AuthorizationScope,
    context: &RequestContext,
...
pub(super) fn valid_anomaly_input(tool: &str, input: &Value) -> bool {
...
pub(super) fn semantic_filter_matches(
...
pub(super) fn finding_skus<'a>(
    finding: &'a BusinessAnomaly,
...
pub(super) fn finding_salesperson<'a>(
    finding: &BusinessAnomaly,
    dataset: &'a BusinessDataset,
) -> Option<&'a str> {
...
pub(super) fn finding_overdue_days(
    finding: &BusinessAnomaly,
    dataset: &BusinessDataset,
) -> Option<u32> {
    let id = finding.primary_resource.id.as_deref()?;
...
pub(super) fn finding_business_date(
    finding: &BusinessAnomaly,
    dataset: &BusinessDataset,
) -> chrono::NaiveDate {
...
pub(super) fn impact_totals(findings: &[BusinessAnomaly]) -> Vec<business_query_contracts::Money> {
...
pub(super) fn legacy_read_result(
    analytics: &BusinessAnalyticsService,
    tool: &str,
    input: &Value,
    scope: &AuthorizationScope,
