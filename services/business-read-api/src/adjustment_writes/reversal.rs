...
pub(super) fn attributed(v: &Value, scope: &AuthorizationScope) -> bool {
    // There is no supplier attribution on these facts. Missing dimensions never
    // satisfy a restricted delegate, even when the creator's scope is broader.
    scope.supplier_ids.is_empty()
        && v["reversalPreview"]["preview"]["facts"]
            .as_array()
            .is_some_and(|rows| {
                rows.iter().all(|row| {
                    [
                        ("customer_id", &scope.customer_ids),
                        ("business_unit_id", &scope.business_unit_ids),
                        ("warehouse_id", &scope.warehouse_ids),
                        ("brand_id", &scope.brand_ids),
                    ]
                    .iter()
                    .all(|(field, allowed)| {
                        allowed.is_empty()
