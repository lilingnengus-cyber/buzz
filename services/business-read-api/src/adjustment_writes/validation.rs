...
pub(super) fn valid_snapshot(v: &Value, kind: &str) -> bool {
    if kind == "operational_adjustment_reversal_intent" {
        return super::reversal::validate(v).is_some();
    }
...
pub(super) fn permits(v: &Value, scope: &AuthorizationScope, kind: &str) -> bool {
    ...
    if kind != "operational_adjustment_post_intent" {
        return if kind == "operational_adjustment_reversal_intent" {
            super::reversal::attributed(v, scope)
        } else {
            super::drafts::attributed(v, scope)
        };
    }
