...
use super::*;
use business_core::b4::model::CreateAdjustmentBatch;
...
pub(super) fn canonical(tool: &str, v: &Value) -> Option<Value> {
    match tool {
...
pub(super) fn attributed(v: &Value, scope: &AuthorizationScope) -> bool {
    // No supplier attribution exists. Restricted grants must not authorize unassigned drafts.
    if !scope.supplier_ids.is_empty() {
        return false;
    }
    let p = &v["draftPreview"]["preview"];
    for (field, old_field, allowed) in [
        ("customerId", "customer_id", &scope.customer_ids),
        (
            "businessUnitId",
            "business_unit_id",
            &scope.business_unit_ids,
        ),
        ("warehouseId", "warehouse_id", &scope.warehouse_ids),
        ("brandId", "brand_id", &scope.brand_ids),
    ] {
        if allowed.is_empty() {
            continue;
        }
        // Old target dimensions are not present in this preview; require explicit old attribution.
        if p["source"]["lines"].as_array().is_some_and(|ls| {
            ls.iter()
                .any(|l| l[old_field].as_str().is_none_or(|s| !allowed.contains(s)))
        }) {
            return false;
        }
        if p["input"]["lines"].as_array().is_none_or(|ls| {
            ls.iter()
                .any(|l| l[field].as_str().is_none_or(|s| !allowed.contains(s)))
        }) {
            return false;
        }
        if field != "warehouseId"
            && p["referencedOrders"].as_array().is_none_or(|os| {
                os.iter()
                    .any(|o| o[field].as_str().is_none_or(|s| !allowed.contains(s)))
            })
        {
            return false;
        }
