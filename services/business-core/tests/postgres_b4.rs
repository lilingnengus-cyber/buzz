...
#[path = "support/operating_snapshot_business_units.rs"]
mod operating_snapshot_business_units;
#[path = "support/operating_snapshot_concurrency.rs"]
mod operating_snapshot_concurrency;
#[path = "support/operating_snapshot_consistency.rs"]
...
async fn b4_postgres_profit_projection_adjustment_reporting_and_concurrency() {
    ...
    operating_snapshot_consistency::verify(&pool, &operations, f.actor, date).await;
    operating_snapshot_concurrency::verify(&pool, &operations, f.actor, date).await;
