...
pub async fn verify(pool: &PgPool, app: &Router, f: &Fixture, order: Uuid, reviewer: Uuid) {
    let batch = fixture::draft(pool, f, order, "adjustment-expiry-draft").await;
    ...
    sqlx::raw_sql("CREATE FUNCTION pause_adjustment_vote() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.operation='chat_document_approval_vote' AND NEW.target_type='operational_adjustment_post_intent' THEN PERFORM pg_advisory_xact_lock(202609207); END IF; RETURN NEW; END $$; CREATE TRIGGER pause_adjustment_vote BEFORE INSERT ON business_core_audit_events FOR EACH ROW EXECUTE FUNCTION pause_adjustment_vote();").execute(pool).await.unwrap();
