...
async fn voter(
    ...
    // This family-specific preview checks current master management authority,
    // exact object and parents. Global types legitimately have no legal entity.
    if command.preview_on(store, tx, actor).await? != *snapshot {
        return Err(StoreError::Conflict);
    }
    Ok(policy_deadline.into_iter().chain(write_deadline).min())
}
...
pub(super) async fn execute(
    ...
    command.lock_target(&mut tx).await?;
    if command.preview_on(store, &mut tx, actor).await? != snapshot {
        return Err(StoreError::Conflict);
    }
    ...
    let current: bool = sqlx::query_scalar("SELECT ($1::timestamptz IS NULL OR $1>clock_timestamp()) AND EXISTS(SELECT 1 FROM business_agent_master_intents WHERE id=$2 AND expires_at>clock_timestamp())")
