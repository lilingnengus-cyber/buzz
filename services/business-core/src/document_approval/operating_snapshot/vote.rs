...
async fn voter(
    ...
    let read_deadline = authority::permission(tx, actor, "management_report:read").await?;
    ...
    // Recompute the requester's report, not the reviewer's distinct personal snapshot.
    if snapshot["ownerUserId"] != json!(creator)
        || command.preview_on(&state.operations, tx, creator).await? != *snapshot
    {
...
pub(super) async fn execute(
    ...
    let current: bool = sqlx::query_scalar("SELECT ($1::timestamptz IS NULL OR $1>clock_timestamp()) AND EXISTS(SELECT 1 FROM business_agent_operating_snapshot_intents WHERE id=$2 AND expires_at>clock_timestamp())")
