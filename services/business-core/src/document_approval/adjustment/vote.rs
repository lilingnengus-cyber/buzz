...
async fn voter(
    ...
    Ok(policy_deadline
        ...
        .chain(creator_preview_deadline)
        .min())
}
...
pub(super) async fn execute(
    ...
    let terminal:bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM business_document_approval_requests WHERE document_type=$1 AND document_id=$2 AND status<>'pending')").bind(kind).bind(id).fetch_one(&mut *tx).await?;
    if terminal {
        return Err(StoreError::Conflict);
    }
    ...
    let current: bool = sqlx::query_scalar("SELECT ($1::timestamptz IS NULL OR $1>clock_timestamp()) AND EXISTS(SELECT 1 FROM business_agent_adjustment_intents WHERE id=$2 AND expires_at>clock_timestamp())")
