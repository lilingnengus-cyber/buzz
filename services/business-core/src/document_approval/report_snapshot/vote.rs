...
async fn voter(
    ...
    if command
        .preview_on(&state.profit_reporting, tx, actor)
        .await?
        != *snapshot
        || command
            .preview_on(&state.profit_reporting, tx, creator)
            .await?
            != *snapshot
    {
...
pub(super) async fn execute(
    ...
    let current: bool = sqlx::query_scalar("SELECT ($1::timestamptz IS NULL OR $1>clock_timestamp()) AND EXISTS(SELECT 1 FROM business_agent_report_snapshot_intents WHERE id=$2 AND expires_at>clock_timestamp())")
