...
pub(super) async fn lock_order(
    ...
    let header = sqlx::query("SELECT o.legal_entity_id FROM sales_orders o JOIN business_customers c ON c.id=o.customer_id JOIN business_units u ON u.id=o.business_unit_id WHERE o.id=$1 AND c.status='active' AND u.status='active' FOR SHARE OF c,u")
