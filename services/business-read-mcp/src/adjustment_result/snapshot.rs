...
pub(super) fn validate(v: &Value, kind: &str) -> Result<(), String> {
    check(v, kind).ok_or_else(|| "Invalid operational adjustment preview".to_string())
}
fn check(v: &Value, kind: &str) -> Option<()> {
    let command: adjustment_tools::AdjustmentPostInput =
        serde_json::from_value(v["input"].clone()).ok()?;
    if command.batch_id.is_nil() || command.expected_version < 1 {
        return None;
    }
    let input = serde_json::to_value(command).ok()?;
