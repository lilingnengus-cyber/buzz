...
入口：企业工作台「售前 CRM」独立分组，与「业务闭环」同级，下设「商机」「跟进记录」「客户联系人」。商机保留 `/#crm`；跟进记录为 `/#crmFollowups`，客户联系人为 `/#crmContacts`，对应嵌入路径 `/embed/crm`、`/embed/crm/followups`、`/embed/crm/contacts`。

企业客户售前采用商机列表和详情页，并提供独立的跟进历史和客户联系人页面。阶段为新线索、沟通中、报价中、已成交、已流失。可以填写潜在客户公司和联系人，或关联有权限访问的已有客户。每次跟进保留沟通内容、作者、时间、当时阶段和下一步；下次跟进日期支持「今天及逾期」筛选。预计金额可为空；已成交/已流失不进入待跟进筛选。成交后可转到现有销售订单模块手工录单。
...
跨商机跟进记录由 `GET /api/v1/crm/followups` 读取，按创建时间倒序显示；联系人由 `GET /api/v1/crm/contacts` 汇总现有商机联系人，不建立重复主数据。只有同一法人、业务单元、关联客户、公司、姓名和联系方式完全相同的记录才合并显示，保留全部关联商机链接。联系人通过关联商机编辑，不提供独立的新建/删除。

两个列表均在服务端先应用 crm:read 和当前数据范围，再搜索及每页 50 条分页。跟进记录可搜沟通内容，联系人可搜联系方式。链接 `/#crm?opportunity=<id>` 打开相应详情；原 `/#crm` 入口继续有效。没有数据库迁移或历史数据改写。

## 2026-09-19 三页面发布验收

正式发布分支 `codex/crm-pages-release` 基于线上 `8dd2301b2`，保留已上线的业务闭环和库存期初入口，网页发布提交 `a0eba3276`。Core 在 `stock-734400865` 源码基线上仅移植 CRM 查询改动，镜像 `shiyue-business-core:crm-registers-20260919`，附加 Compose `compose.crm-registers-20260919.yml`；无迁移。网页版本 `business-web-a0eba3276-139f448030c2`，JS SHA256 `52b9f956c4b87500961419798315e10734254775d96dc24a3ea172126bbe8f54`。发布脚本核验静态资源、IAM/Core 健康均成功。

31 项网页单元测试、扩展 CRM Playwright 流程（独立跟进/联系人页面、刷新与详情链接）、类型检查、展示格式检查、构建、Core 严格 Clippy、真实 PostgreSQL 权限/搜索/分页/权限撤销测试通过。未运行全仓 just ci，未创建 PR。Pacioli 生产客户端实际点击三个导航，读取「杭州某公司／张经理／年度采购项目」及「已发送方案，等待客户反馈」记录，从联系人链接返回正确商机，保留 2026-09-21 跟进日期。本轮仅只读验收，没有新增生产业务记录。
