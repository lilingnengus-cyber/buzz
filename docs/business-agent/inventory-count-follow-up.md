...
## 2026-09-20 分页预览与默认容量原生验收

新增固定只读工具 `get_inventory_count_approval_preview`，候选共 121 工具，沿用 `inventory:read`，不新增委托能力。输入必须包含意图 UUID、四种固定 documentType 之一和完整 previewHash；offset 0–500，limit 1–20。每次读取都让 Core 重读当前完整快照，并核对意图 ID、签名摘要、确认/拒绝指令与 trace，再对整张快照（含页外行）执行委托范围检查。审批执行仍使用完整服务端快照，分页不会缩小实际审批范围。

准备结果和只读预览每页最多 20 行，保留该页的全部行字段、数量、成本与影响，命令中的 SKU/实盘行与可见行对齐。`previewPagination` 明示总行数、当前 offset、nextOffset/hasMore；`previewHashScope=complete_server_snapshot` 明示摘要不是单页摘要。Host 与工具说明要求逐页核对，禁止把首页说成整张单据。`get_inventory_count` 同步分页，后续页必须携带首个版本 expectedVersion，源单变更返回冲突；行数分页不会被误报为多个单据待消歧。

容量实测（500 行、SKU 名称 200 个中文字符）：创建准备 9,875 字节，详情首屏 22,863 字节，录入准备 15,681 字节，过账准备 14,677 字节；后续预览约 15,938 字节。测试将每份响应上限收紧至默认 Agent 文本预算 50 KiB。25 页详情与 25 页审批预览遍历 500 个不重复行 ID，逐行实盘值与摘要一致；提交后旧版本详情和旧审批预览被拒绝。最终真实 Core/PostgreSQL 过账数量 500、价值 3500，证明分页显示未把实际执行缩成 20 行。

原生 buzz-agent + MCP 121 工具 + 本地模拟模型/服务验收：工具输入包含实际 500 行；使用默认 MCP 131072 字节、Agent 文本 51200 字节、上下文 200000 预算，首轮完整分页响应与后续第 25 页均到达模型请求，JSON 内容、摘要及分页保持一致。无需先前的 1 MiB 或虚拟大上下文配置。此项验证原生协议和传输，不等于真实模型自主遍历或生产聊天验收。修复过程中发现 recordedActualQuantity=null 被读取结果校验误拒绝，现明确允许未录入空值，仍拒绝 JSON 数字数量。
